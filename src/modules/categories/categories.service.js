...
const createCategory = async (payload) => {
	const existingCategory = await categoriesRepository.findCategoryByName(
		payload.name,
	);
...
const getCategories = async (queryParams) => {
	...
	const filters = {
		is_active:
			typeof queryParams.is_active === "boolean"
...
const updateCategory = async (id, payload) => {
	...
	if (
		payload.name &&
		payload.name.toLowerCase() !== currentCategory.name.toLowerCase()
	) {
		const existingCategory = await categoriesRepository.findCategoryByName(
			payload.name,
		);
