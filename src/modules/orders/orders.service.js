...
const ensureOrderHasItems = async (client, orderId) => {
	const items = await orderItemsRepository.listOrderItemsByOrderId(
		client,
		orderId,
	);
...
const ensureActiveDailySession = async (client, authenticatedUser) => {
	const activeSession =
		await dailySessionsRepository.findActiveDailySession(client);
	...
	const todaySession = await dailySessionsRepository.findDailySessionByDate(
		client,
		today,
	);
...
const syncOrderPreparationStatus = async (client, orderId) => {
	const preparation_status =
		await orderItemsRepository.calculateOrderPreparationStatus(client, orderId);
...
const createOrder = async (payload, authenticatedUser) => {
	return withTransaction(async (client) => {
		const dailySession = await ensureActiveDailySession(
			client,
			authenticatedUser,
		);

		const order_number = await ordersRepository.getNextOrderNumber(
			client,
			dailySession.id,
		);
...
const addOrderItem = async (orderId, payload) => {
	return withTransaction(async (client) => {
		...
		const item = await orderItemsRepository.findOrderItemById(
			client,
			created.id,
		);
...
const updateOrderItem = async (orderId, itemId, payload) => {
	return withTransaction(async (client) => {
		...
		const updatedItem = await orderItemsRepository.findOrderItemById(
			client,
			itemId,
		);
		const updatedOrder = await ordersRepository.findOrderById(client, orderId);
...
const updateOrderItemPreparationStatus = async (orderId, itemId, payload) => {
	return withTransaction(async (client) => {
		...
		if (item.preparation_status === payload.preparation_status) {
			throw new BadRequestError(
				`Order item is already ${payload.preparation_status}`,
			);
		}
