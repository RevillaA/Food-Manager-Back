...
const attachSaleItems = async (client, sales = []) => {
	...
	const items = await saleItemsRepository.listSaleItemsBySaleIds(
		client,
		saleIds,
	);
...
const createSale = async (payload, authenticatedUser) => {
	return withTransaction(async (client) => {
		const order = await ordersRepository.findOrderById(
			client,
			payload.order_id,
		);
		...
		const existingSale = await salesRepository.findSaleByOrderId(
			client,
			payload.order_id,
		);
		...
		const orderItems = await orderItemsRepository.listOrderItemsByOrderId(
			client,
			payload.order_id,
		);
		...
		const sale_number = await salesRepository.getNextSaleNumber(
			client,
			order.daily_session_id,
		);
		...
		const saleItems = await saleItemsRepository.listSaleItemsBySaleId(
			client,
			createdSale.id,
		);
...
const getSales = async (queryParams) => {
	...
	try {
		...
		const data = queryParams.include_items
			? await attachSaleItems(client, sales)
			: sales;
...
const getSalesByDay = async (queryParams) => {
	...
	try {
		...
		const [sales, total] = await Promise.all([
			salesRepository.listSalesBySessionDateRange(client, {
				limit,
				offset,
				...filters,
			}),
